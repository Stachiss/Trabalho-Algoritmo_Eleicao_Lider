...
class Node:
    ...
    def start_election(self):
        ...
        if chosen_node.thread.is_alive():
            ...
            print(f"\n Node {chosen_node.id} nao respondeu. Node {self.id} eh o novo coordenador.")
            self.coordinator = self
        else:
    ...
    # Desliga o nó.
    def shutdown(self):
        print(f"Node {self.id} falhou. A eleicao comecou.")
        # Remove este nó da lista de nós conhecidos de cada nó na rede.
...
if __name__ == "__main__":
    ...
    # Desliga o nó de maior ID.
    max_id_node = max(nodes, key=lambda node: node.id)
    max_id_node.shutdown()

    # Inicia as threads de todos os outros nós.
    for node in nodes:
        if node is not max_id_node:
            node.start()
